# Scaffolding generated by Members <%= members_get_full_version_string %>

module Members
  class <%= class_name.pluralize %>Controller < Members::MembersController
  
    ## optional filters for defining usage according to Members::AdminUser access_levels
    # before_filter :needs_admin, :except => [:action1, :action2]
    # before_filter :needs_admin_or_current_user, :only => [:action1, :action2]
  <% unless @no_index %>
    def index
      @members_page_title = '<%= plural_name.humanize.capitalize %>'
      @<%= plural_name %> = <%= class_name %>.order(sort_order(:<%= attributes[0].name %>)).paginate :page => params[:page]
    end
  <% end %>
    def show
      @members_page_title = 'View <%= singular_name.humanize.downcase %>'
      @<%= singular_name %> = <%= class_name %>.find params[:id]
    end
  <% unless @read_only %>
    def new
      @members_page_title = 'Add a new <%= singular_name.humanize.downcase %>'
    ...
    def create
      ...
      if @<%= singular_name %>.save
        ...
        redirect_to members_<%= @plural_route %>_path
      else
    ...
    def update
      @members_page_title = 'Update <%= singular_name.humanize.downcase %>'
